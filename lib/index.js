      ...
      case 'object':
        ...
        if (!def) {
          ...
          if (def.length == 2 && typeof def[0] == 'number') {
            // array
            if (def[0] == ARRAY) {
              return this.offsets.array + this.fmtDef(def[1], depth + 1); // nullable
            } else if (def[0] == NULLABLE) {
              let subDef = this.fmtDef(def[1], depth + 1); // primitive
              ...
              return TYPED_ARRAY_OFFSET + this.fmtDef(def[1], depth + 1); // object
            } else if (def[0] == OBJECT) {
              return this.offsets.object + this.fmtDef(def[1], depth + 1); // variable def
