...
class Tbjson {
  ...
  registerPseudoPrototype(id, def) {
    let code = this.protoRefs[id]; // already registered
  ...
  serializeToFile(filename, obj) {
    return new Promise((res, rej) => {
      try {
        ...
        let writeStream = fs__default["default"].createWriteStream(tempFilename, 'binary');
        this.serializeToStream(writeStream, obj);
        writeStream.end(); // write the final file

        writeStream = fs__default["default"].createWriteStream(filename, 'binary'); // write the header

        writeStream.write(this.getHeaderAsBuffer()); // pipe the tmp file to the final file

        let readStream = fs__default["default"].createReadStream(tempFilename, 'binary');
        readStream.pipe(writeStream);
        readStream.on('end', () => {
          // cleanup
          fs__default["default"].unlinkSync(tempFilename);
          res();
        });
      } catch (e) {
  ...
  parseFileAsBuffer(filename, selector = null) {
    try {
      return this.parseBuffer(fs__default["default"].readFileSync(filename), selector);
    } catch (e) {
  ...
  async parseFileAsStream(filename, selector = null) {
    try {
      return await this.parseStream(fs__default["default"].createReadStream(filename), selector);
    } catch (e) {
      e.message = `Tbjson failed to parse "${filename}": ` + e.message;
      throw e;
    }
      ...
      case 'object':
        ...
        if (!def) {
          ...
          if (def.length == 2 && typeof def[0] == 'number' && def[0] > 10) {
            // array
            if (def[0] == ARRAY) {
              return this.offsets.array + this.fmtDef(def[1], depth + 1, ARRAY); // nullable
            } else if (def[0] == NULLABLE) {
              let subDef = this.fmtDef(def[1], depth + 1, NULLABLE); // primitive
              ...
              return TYPED_ARRAY_OFFSET + this.fmtDef(def[1], depth + 1, TYPED_ARRAY); // object
            } else if (def[0] == OBJECT) {
              return this.offsets.object + this.fmtDef(def[1], depth + 1, OBJECT); // variable def
            ...
            if (insideOf == ARRAY || insideOf == OBJECT) {
              ...
              return code; // just return the def
            } else {
              return fmtDef;
            }
          } // simple object
  ...
  if (prototype && (typeof prototype == 'function' || typeof prototype == 'object')) {
    ...
    if (Array.isArray(obj) && isArray) {
      ...
      if (tbjson && tbjson.definition) {
        ...
        if (tbjson.cast) {
          return Tbjson.cast(obj, tbjson.cast(obj), free, definitions); // use the passed prototype
        } else {
