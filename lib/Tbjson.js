  ...
  registerPrototype(prototype) {
    ...
    if (!this.protos[code] || !this.protos[code].definition) {
      ...
      if (prototype.definition) {
        ...
        let parentCode = parent ? this.registerPrototype(parent) : null; // set the prototype

        this.protos[code] = new Prototype(this.fmtDef(prototype.definition), prototype.prototype, parentCode, prototype.noInherit); // set empty if there is no prototype
      } else {
        this.protos[code] = {};
      }
    }
  ...
  parse(def, prototype) {
    ...
    if (typeof def == 'number') {
      ...
      if (def < NULLABLE_OFFSET) {
        ...
        let proto = this.protos[def]; // defined

        if (proto.definition) {
          return this.parse(proto.definition, proto.prototype); // undefined
        } else {
