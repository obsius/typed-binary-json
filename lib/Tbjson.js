...
const OBJECT = 12;
const NULLABLE = 13;
const TYPED_ARRAY = 14; // primitive sizes
const SIZE_INT8 = 1;
const SIZE_UINT8 = 1;
const SIZE_INT16 = 2;
const SIZE_UINT16 = 2;
...
const PROTOTYPE_OFFSET = 64;
const ARRAY_OFFSET = 256;
...
class Tbjson {
  ...
  constructor(types = [], prototypes = [], options = {}) {
    ...
    _defineProperty(this, "nextProtoCode", PROTOTYPE_OFFSET);
  ...
  parseBuffer(buffer, selector = null) {
    try {
      ...
      if (this.reader.readFixedLengthString(STRING, SIZE_MAGIC_NUMBER) != MAGIC_NUMBER) {
      ...
      case 'object':
        ...
        if (!def) {
          ...
          if (def.length == 2 && def[0] == ARRAY) {
            return ARRAY_OFFSET + this.fmtDef(def[1]); // nullable primitive
          } else if (def.length == 2 && def[0] == NULLABLE) {
            return NULLABLE_OFFSET + this.fmtDef(def[1]); // primitive typed array
  ...
  serializeDef(obj, def) {
    ...
    if (!def) {
      ...
      this.writer.write(UINT32, code); // write the obj
    ...
    if (typeof def == 'number') {
      ...
      if (def < NULLABLE_OFFSET) {
        // an unknown object
        if (def == OBJECT) {
          this.serializeDef(obj); // primitive
        } else {
      ...
      } else if (def < PROTOTYPE_OFFSET) ; else if (def < ARRAY_OFFSET) {
        ...
        if (!obj) {
          ...
          if (obj.constructor) {
            ...
            if (obj.constructor.tbjson) {
              ...
              let code = this.registerPrototype(obj.constructor); // process the prototype definition

              this.serializeDef(obj, this.protos[code].definition);
              return code; // might be a known tbjson prototype
            } else {
  ...
  parseAtSelection(def, selector, path = [], prototype) {
    ...
    if (typeof def == 'number' && def == OBJECT) {
      return this.parseAtSelection(this.objs[this.reader.read(UINT32)], selector, path); // forward a known prototype
    } else if (typeof def == 'number' && def >= PROTOTYPE_OFFSET && def < ARRAY_OFFSET) {
      let proto = this.protos[def];
      return this.parseAtSelection(proto.definition ? proto.definition : this.objs[this.reader.read(UINT32)], selector, path, proto.prototype); // control the object path
    ...
    if (typeof def == 'number') {
      ...
      if (def < NULLABLE_OFFSET) {
        ...
        if (def == NULL) {
          ...
          return this.parse(this.objs[this.reader.read(UINT32)]); // primitive
        } else {
        ...
        let proto = this.protos[def];
        return this.parse(proto.definition ? proto.definition : this.objs[this.reader.read(UINT32)], proto.prototype); // variable-length fixed typed array 
      } else {
