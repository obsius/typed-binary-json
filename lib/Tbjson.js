...
const UNKNOWN = 15; // primitive sizes			
const SIZE_INT8 = 1;
const SIZE_UINT8 = 1;
const SIZE_INT16 = 2;
const SIZE_UINT16 = 2;
...
const NULLABLE_PROTOTYPE_OFFSET = 160;
...
class Tbjson {
    ...
    switch (typeof def) {
      ...
      case 'object':
        ...
        if (!def) {
          ...
          if (def.length == 2) {
            if (def[0] == ARRAY) {
              return ARRAY_OFFSET + this.fmtDef(def[1]); // nullable
  ...
  serializeDef(obj, def) {
    ...
    if (!def) {
      ...
      this.writer.write(UINT16, code); // write the obj
...
Tbjson.cast = (obj, prototype = {}, definitions = {}) => {
  ...
  if (typeof obj == 'object') {
    ...
    if (Array.isArray(obj)) {
      ...
      return typedObj; // uniform value object
      ...
      return typedObj; // nullable object
    } else if (isArrayTypeDef && prototype[0] == NULLABLE) {
      return obj == null ? null : Tbjson.cast(obj, prototype[1], definitions); // object
    } else if (prototype.tbjson && prototype.tbjson.definition) {
      let definition; // use map
